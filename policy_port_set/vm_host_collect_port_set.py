#!/usr/bin/env python3
"""Standalone PC collect: VM / NIC / host / cluster + Atlas port-set → IP map.

Same folder as update_policy_port_sets.py. Writes /tmp by default.

  python3 vm_host_collect_port_set.py
  python3 update_policy_port_sets.py --from-pc

Writes:
  /tmp/vms_port_set.json   one row per NIC, with port_sets[].ip_list
  /tmp/port_set_ips.json   port_set_uuid -> {name, ip_list, nics}

--from-pc runs this on its own, then writes /tmp/policy.json with every
port_set unmarshalled to ip_list.
"""
...
DEFAULT_OUT_DIR = "/tmp"
DEFAULT_OUT = os.path.join(DEFAULT_OUT_DIR, "vms_port_set.json")
...
def map_nic(row):
  return {
      ...
      "hypervisor_specific_id": as_uuid(first_attr(
          row, "hypervisor_specific_id", "hypervisorSpecificId")),
...
def port_sets_for_nic(nic, nic_to_port_sets):
  """Atlas virtual_nic_uuid_list is often hypervisor_specific_id, not IDF uuid."""
  out, seen = [], set()
  for key in (
      nic.get("ext_id"), nic.get("hypervisor_specific_id"),
      nic.get("atlas_nic_uuid")):
    uid = as_uuid(key) or str(key or "").strip()
    if not uid:
      continue
    for detail in nic_to_port_sets.get(uid) or []:
      ps = as_uuid(detail.get("uuid")) or detail.get("uuid")
...
def inventory_rows(vms, nics, hosts, clusters, subnets, vpcs, categories, caps,
  ...
  for vm in vms:
    ...
    for nic in vm_nics:
      ...
      nic_ps = port_sets_for_nic(nic, nic_to_port_sets)
      rows.append({
          "vm": vm.get("name") or "",
          "vm_uuid": vm_uuid,
          "nic_uuid": nic.get("ext_id") or "",
          "atlas_nic_uuid": nic.get("hypervisor_specific_id") or "",
          "mac": nic.get("mac") or "",
          ...
          "vpc_type": vpc_type,
          "port_set": ",".join(
              (d.get("name") or d.get("uuid") or "") for d in nic_ps),
          "port_set_uuid": ",".join(
              (d.get("uuid") or "") for d in nic_ps),
          "port_sets": list(nic_ps),
      })
  return rows


def split_row_ips(row):
  out = []
  seen = set()
...
def build_port_set_ip_map(rows):
  """port_set_uuid -> name, ip_list, nics. Atlas nic may be hypervisor_specific_id."""
  mapping = {}
  for row in rows or []:
    ips = split_row_ips(row)
    nic = {
        "ip": ",".join(ips),
        "mac": row.get("mac") or "",
        "vm": row.get("vm") or "",
        "vm_uuid": row.get("vm_uuid") or "",
        "nic_uuid": row.get("nic_uuid") or "",
        "atlas_nic_uuid": row.get("atlas_nic_uuid") or "",
    }
    for ps in row.get("port_sets") or []:
      if not isinstance(ps, dict):
        continue
      uid = as_uuid(ps.get("uuid"))
      if not uid:
        continue
      rec = mapping.setdefault(uid, {
          "uuid": uid,
          "name": str(ps.get("name") or ""),
          "ip_list": [],
          "nics": [],
      })
      if ps.get("name") and not rec["name"]:
        rec["name"] = str(ps.get("name") or "")
      for ip in ips:
        if ip not in rec["ip_list"]:
          rec["ip_list"].append(ip)
      rec["nics"].append(nic)
  return mapping


def attach_port_set_ip_lists(rows, mapping):
  mapping = mapping or {}
  for row in rows or []:
    for ps in row.get("port_sets") or []:
      if not isinstance(ps, dict):
        continue
      uid = as_uuid(ps.get("uuid"))
      rec = mapping.get(uid) or {}
      ps["ip_list"] = list(rec.get("ip_list") or [])
  return rows
...
def self_test():
  uid = "917386ac-6978-5da1-91a6-3782ffebd275"
  rows = [{
      "ip": "142.58.233.98,10.28.2.149",
      "mac": "00:50:56:a9:47:db",
      "vm": "src",
      "vm_uuid": "11111111-1111-1111-1111-111111111111",
      "nic_uuid": "48677709-0000-0000-0000-000000000001",
      "atlas_nic_uuid": "25f99090-0000-0000-0000-000000000001",
      "port_sets": [{"uuid": uid, "name": "EG-T"}],
  }]
  mapping = build_port_set_ip_map(rows)
  rec = mapping.get(uid)
  if not rec or rec.get("ip_list") != ["142.58.233.98", "10.28.2.149"]:
    raise SystemExit("port_set ip map failed: %s" % rec)
  attach_port_set_ip_lists(rows, mapping)
  if rows[0]["port_sets"][0].get("ip_list") != rec["ip_list"]:
    raise SystemExit("nic port_sets ip_list missing")
  print("self-test ok port_set=%s ips=%s" % (uid, rec["ip_list"]))
...
def main(argv=None):
  ...
  ap.add_argument(
      ...
      help="Run a stdlib-only port_set -> ip_list check and exit")
  ap.add_argument(
      "--out", default=DEFAULT_OUT,
      help="NIC inventory JSON. Default: %s" % DEFAULT_OUT)
  ap.add_argument(
      "--port_set_ips", default="",
      help="port_set_uuid -> ip_list JSON (default: <out dir>/port_set_ips.json)")
  ...
  if not rows:
    ...
    return 2
  ps_map = build_port_set_ip_map(rows)
  attach_port_set_ip_lists(rows, ps_map)
  out_path = os.path.abspath(args.out)
  parent = os.path.dirname(out_path)
  if parent:
    os.makedirs(parent, exist_ok=True)
  ps_path = os.path.abspath(args.port_set_ips) if args.port_set_ips else (
      os.path.join(parent or ".", "port_set_ips.json"))
  with open(out_path, "w") as handle:
    json.dump(rows, handle, indent=2)
    handle.write("\n")
  with open(ps_path, "w") as handle:
    json.dump(ps_map, handle, indent=2)
    handle.write("\n")
  ...
  with_ips = sum(1 for rec in ps_map.values() if rec.get("ip_list"))
  log("wrote %s rows=%s with_vm_cat=%s with_subnet_cat=%s with_vpc_cat=%s "
      "with_vpc=%s with_port_set=%s" % (
          ...
          with_vpc, with_ps))
  log("wrote %s port_sets=%s with_ip_list=%s" % (
      ps_path, len(ps_map), with_ips))
  return 0
