...
def _is_pressure(row: dict[str, Any]) -> bool:
  message = str(row.get('message', '')).lower()
  return any([
    row.get('cpu_pct', 0) > 90,
    row.get('memory_pct', 0) > 90,
    row.get('load_avg', 0) > row.get('load_threshold', 0),
    'oom' in message,
    'panic' in message,
    'soft lockup' in message,
    'hard lockup' in message,
  ])


def run(db_client: Any, context: dict[str, Any]) -> dict[str, Any]:
  rows = []
  rows.extend(_search_rows(db_client, context, ['oom', 'panic', 'lockup', 'hung task']))
  ...
  if not rows:
    return _result('network-host-pressure', 'NO_HOST_PRESSURE_EVIDENCE', context, [], [], ['No host pressure evidence'])

  pressure = [row for row in rows if _is_pressure(row)]
  if pressure:
    return _result('network-host-pressure', 'HOST_PRESSURE_FOUND', context, [{'type': 'HOST_PRESSURE', 'count': len(pressure)}], pressure, [])
  return _result('network-host-pressure', 'NO_HOST_PRESSURE_FOUND', context, [], rows, [])
